...
public class Netalyzr {
   ...
   private boolean init_l10n() {
      ...
      for(int var7 = 0; var7 < var5.size(); ++var7) {
         ...
         if (var9 != null && (var9.getProtocol().toLowerCase().equals("jar") || var9.getProtocol().toLowerCase().equals("file"))) {
            try {
   ...
   void init_impl() {
      ...
      if (!this.init_l10n()) {
         ...
         //this.initSucceeded = true; need to be true but must make test first
         this.makeTests();
   ...
   void makeTests() {
         ...
         int runImpl() throws IOException {
            if (!Netalyzr.this.canDoUnrestrictedLookup) {
               ...
               for(int var4 = 0; var4 < this.names.length; ++var4) {
                  ...
                  final int varf4 = var4;
                  Thread var6 = new Thread() {
                     public void run() {
                        results[varf4] = fetchName(names[varf4]);
                     }
                  };
                  var6.start();
      ...
      this.tests.add(new Netalyzr.Test("checkHTTPCache") {
         ...
         DigestResult getImageDigest(String var1) throws MalformedURLException, IOException {
            ...
            DigestResult var7 = new DigestResult(var4.getResponseCode(), var5);
         ...
         int compareImageDigests(String var1, String var2) throws MalformedURLException, IOException {
            DigestResult var3 = this.getImageDigest(var1);
            DigestResult var4 = this.getImageDigest(var1);
      ...
      this.tests.add(new Netalyzr.Test("checkDNSResolvers") {
         //resData = new ResolverData(var1.addr, "Nat" + var2); something goign on here
         //resolverInfo = new ResolverData(var1.addr, "Nat" + var2);
      ...
      this.tests.add(new Netalyzr.Test("checkDNSNAT") {
         ...
         int runImpl() throws IOException {
            if (Netalyzr.this.foundNatAddrs.size() == 0 && !Netalyzr.this.isNatted()) {
               ...
               if (Netalyzr.this.foundNatAddrs.size() == 0) {
                  ...
                  this.candidateNats.add(new NatDevice(var2 + ".1"));
                  this.candidateNats.add(new NatDevice(var2 + ".254"));
               } else {
                  Netalyzr.this.debug("Using UPnP-determined address(es)");

                  for(var3 = 0; var3 < Netalyzr.this.foundNatAddrs.size(); ++var3) {
                     this.candidateNats.add(new NatDevice((String)Netalyzr.this.foundNatAddrs.get(var3)));
                  }
               }

               for(var3 = 0; var3 < this.candidateNats.size(); ++var3) {
                  this.probeNat((NatDevice)this.candidateNats.get(var3), var3);
               }
         ...
         void probeNat(NatDevice var1, int var2) throws IOException {
            
//where resolver class was
         ...
         String getPostResults() {
            ...
            for(int var2 = 0; var2 < this.candidateNats.size(); ++var2) {
               NatDevice var3 = (NatDevice)this.candidateNats.get(var2);
